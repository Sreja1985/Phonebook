...
export default function Location() {
    ...
    const onSuggestHandler = (text, name) => {        
            setText(text);
            setSuggestions([]);            
            onSubmit(name.id);              
    };
    ...
    return (
        <div className="content-page">          
                ...
                {suggestions && suggestions.slice(0, 5).map((suggestion, key) => 
                    ...
                    value={suggestion.id} 
                    key = {key}                    
                    onClick={() => onSuggestHandler(suggestion.name, suggestion)}                                                   
                    > 
                    {suggestion.name} {suggestion.Department.alias} {suggestion.Region.regionTitle}
                    </div>
                )} 
